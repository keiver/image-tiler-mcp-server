...
**URL capture:** Requires Chrome/Chromium installed. Set `CHROME_PATH` env var to override auto-detection. URL capture supports `http:` and `https:` URLs; `sourceUrl` image downloads support both -- `https:` uses SSRF filtering, `http:` bypasses it (intended for local dev servers). Pages taller than 16,384px are automatically scroll-stitched.
